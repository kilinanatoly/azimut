    ...
    public function upload($cat_id)
    {
        if (!empty($this->imageFile)){
            if ($this->validate()) {
                ...
                $this->imageFile->saveAs('img/cats_images/'.$cat_id.'/'. $image_name);
                $model = ImagesForCats::findOne(['cat_id'=>$cat_id]);
                if (!empty($model)){
                    if (file_exists("../web/img/cats_images/".$cat_id.'/'.$model->url)){
                        unlink("../web/img/cats_images/".$cat_id.'/'.$model->url);
                    }
                }
    ...
    public function upload1($arenda_type_id)
    {
        if (!empty($this->imageFile)){
            if ($this->validate()) {
                ...
                $image_name = md5($this->imageFile->baseName).date('His') . '.' . $this->imageFile->extension;
                $this->imageFile->saveAs('img/arenda_types_images/'.$arenda_type_id.'/'. $image_name);
                $model = ImagesForArendaTypes::findOne(['arenda_type_id'=>$arenda_type_id]);
                if (!empty($model)){
                    if (file_exists("../web/img/arenda_types_images/".$arenda_type_id.'/'.$model->url)){
                        unlink("../web/img/arenda_types_images/".$arenda_type_id.'/'.$model->url);
                    }
                }
            ...
            if ($this->validate()) {
                ...
                if (!empty($model)){
                    if (file_exists("../web/img/arenda_types_images/".$arenda_type_id.'/'.$model->url_black)){
                        unlink("../web/img/arenda_types_images/".$arenda_type_id.'/'.$model->url_black);
                    }
                }
                if (empty($model)) $model = new ImagesForArendaTypes();
